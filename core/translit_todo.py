...
_CHAR_ROM_TO_TODO = {
    ...
    "p": "ᡌ",
    "h": "ᡍ",
    "\x01": "ᡍ",   # x и k из транслитерации, в обход таблицы диграфов
    "\x02": "ᡗ",   # k перед задней гласной — отдельная буква TODO KA
    "g": "ᡎ",
    "m": "ᡏ",
    "l": "ᠯ",
    "s": "ᠰ",
...
_SPECIAL_TODO_TO_ROM = {
    "ᡘ": "kh",
    "ᡀ": "lh",
    "ᡗ": "k",    # TODO KA — та самая k перед задней гласной
    "ᡙ": "h",
}

# Служебный символ для x/k: см. комментарий в _TRANSLIT_TO_ROM_MULTI.
# Нужен любой знак, которого заведомо нет в тексте и который не образует
# пары с соседями в таблице диграфов.
_H_MARK = "\x01"

# Второй служебный символ — для k перед задними гласными (а/о/у).
#
# В тодо бичиг это разные буквы: ᡍ (U+184D, TODO QA) — задний ряд, ᡗ
# (U+1857, TODO KA) — передний. Обычно они в дополнительном распределении:
# x идёт с задними гласными, k с передними, и обе спокойно писались через
# ᡍ. Но в заимствованиях k попадает перед задней гласной — кару, karou, —
# и тогда ᡍ прочтётся как q. Нужна явная ᡗ: ᡗᠠᠷᡆᡇ, а не ᡍᠠᠷᡆᡇ.
# В словаре таких слов 219 из 25 664.
_K_MARK = "\x02"

# Задние гласные на этом шаге уже приведены к ASCII: a, o, u. Передние
# огублённые стали заглавными O/U, так что строчные [aou] — ровно задний ряд.
_K_BEFORE_BACK = re.compile(r"k(?=[aou])")
...
_TRANSLIT_TO_ROM_MULTI = [
    ...
    ("š", "S"),
    ("ǰ", "j"),
    # x и k ведут в одну и ту же букву тодо бичиг — ᡍ (U+184D).
    #
    # ВАЖНО, почему через служебный символ, а не через "h", как было
    # раньше. Ниже, в _rom_to_todo, сначала отрабатывает таблица
    # _REPLACEMENTS с двухбуквенными сочетаниями из JS-раскладки: lh, gh,
    # kh, ch, zh. Если x превратить в "h", то в слове bolxu возникает
    # последовательность "lh", и она схлопывается в отдельную букву ᡀ:
    # получалось ᡋᡆᡀᡇ вместо ᡋᡆᠯᡍᡇ. Так ломались все слова, где x или k
    # стоят после l или g, — 1391 слово из 25664 в словаре.
    #
    # Служебный символ \x01 ни с чем не сцепляется, поэтому до таблицы
    # диграфов доходит нетронутым, а в _CHAR_ROM_TO_TODO для него заведена
    # прямая запись в ᡍ. Настоящие lh/gh/kh в транслитерации проекта не
    # встречаются ни разу, так что терять нечего.
    (_K_BEFORE_BACK, _K_MARK),
    ("k", _H_MARK),
    ("x", _H_MARK),
...
def translit_to_rom(text):
    ...
    for src, dst in _TRANSLIT_TO_ROM_MULTI:
        # элементом таблицы может быть и готовая регулярка — так записаны
        # правила, которым нужен взгляд на соседнюю букву
        if hasattr(src, "sub"):
...
# =============================================================================
#
# Тире, набранное в исходном тексте (— или –), до таблиц не доходит: в них
# его нет, и в картинку уезжал пустой квадрат. Заменяем его на U+FE31 —
# «презентационную форму вертикального тире». Название обманчиво: в этом
# шрифте знак нарисован горизонтальной чертой, как и все буквы, а
# вертикальным становится после поворота строки — ровно то, что нужно.
#
# Отделяем пробелами с обеих сторон: без них тире прилипает к соседнему
# слову и читается как часть стержня.
#
# Дефис внутри слова сюда не попадает — он остаётся границей суффикса и
# ниже превращается в узкий неразрывный пробел.
VERTICAL_DASH = "\uFE31"

# длинное, короткое, цифровое тире, горизонтальная черта, минус — и
# отдельно стоящий дефис, окружённый пробелами: его тоже считаем тире
_DASH_RE = re.compile(r"[ \t]*(?:[\u2012-\u2015\u2212]|(?<=\s)-(?=\s))[ \t]*")


def _dashes_to_vertical(text):
    return _DASH_RE.sub(f" {VERTICAL_DASH} ", text)
...
def translit_to_todo(text):
    ...
    """
    lines = _dashes_to_vertical(text).split("\n")
